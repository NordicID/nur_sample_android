...
public class NurDeviceListActivity extends Activity {
    public static final String TYPE_LIST_PARAM = "TYPE_LIST";
   ...
   // private BluetoothAdapter mBtAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        params = getIntent().getStringArrayListExtra(TYPE_LIST_PARAM);
    ...
    public static void startDeviceRequest(Activity activity, int devMask) throws InvalidParameterException
    {
        ...
        newIntent.putExtra(NurDeviceListActivity.TYPE_LIST_PARAM, devMask & ALL_DEVICES);

        activity.startActivityForResult(newIntent, NurDeviceListActivity.REQUEST_SELECT_DEVICE);
    }
    public static void saveSettings(Context ctx, String settingsName, String connectionType, String connectionAddress)
    {
        if (ctx != null && settingsName != null && settingsName.length() > 0) {
            SharedPreferences pref = ctx.getSharedPreferences(settingsName, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = pref.edit();
            editor.putString("connType", connectionType);
            editor.putString("connAddr", connectionAddress);
            editor.commit();
        }
    }
}
